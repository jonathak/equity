...
class Array
  ...
  def conversions_helper
    ...
    if size == 1
      security = first.s
      [[security.id,(company.liq_pref - security.liq_payout) + (security.cap/security.percent)]]
    else
      x_shift = company.liq_pref - securities.map(&:liq_payout).sum
      x_max = securities.map{|s| s.cap/s.percent}.max + x_shift
      s_max = securities.select{|s| (s.cap/s.percent + x_shift) == x_max}.map(&:id).first
